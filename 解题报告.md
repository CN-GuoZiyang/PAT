...
判断序列是否是二叉搜索树先根序列或者镜像二叉搜索树的先根序列。

判断方法就是将先序序列按照二叉搜索数的特性转为后序序列，判断转换后的节点数是否相等即可。
...
寻找和为某个值的子序列，如果没有，就找大于等于该值的和的最小值的子序列，并输出。

就是暴力搜索，固定左端，寻找右端，寻找右端的时候用二分法，否则会超时。
...
在一个圈上找出最短的距离，比较简单，只有两种走法，正着走或者反着走，两种方法的距离和为圈的长度。

如果每次都循环累加一遍的话，测试点三会超时，这时，就需要在读入的时候就记录每个点到点一的距离，这样计算得到结果即为两个累加距离的差。
...
### 1047 Student List for Course (25point(s))

需要注意的一个点就是每个课程的学生的姓名需要按照字母顺序排列

学生的姓名如果使用string来存储，cin、cout来输入输出的话会超时，只能使用char数组。在cmp中比较两个char的时候需要使用`strcmp()`，返回的是0、1或-1，而`cmp()`函数返回的是bool，需要加工一下。
...
开一个数组记录每个面值的硬币出现的次数即可。

有一个BUG，就是测试点三的测试用例貌似给了面值超过五百的硬币，把数组开大一点即可。
...
![20181220100453559.png](https://img.guoziyang.top/images/2020/03/09/20181220100453559.png)
...
很简单的一个题目，从第一个字符串中减去所有出现在第二个字符串里的字符，输出。

为ASCII码建立一个bool数组，所有在第二个字符串里出现的字符设置为true，接着遍历比较输出即可。
...
栈操作模拟，入栈顺序是确定的，我们只需要模拟这个过程，从1到N入栈，如果遇到题目所给序列，那么即刻出栈，遍历到最后的结果应该是空栈空序列，刚好满足。
...
坑在于，有一些节点可能不在给定开头的链表中，输出的时候需要剔除，并计算剔除后链表的长度。
...
题目不难，深搜 + 剪枝即可解决，注意输出结果的时候需要输出对是weight，以及对结果的顺序有要求，对`vector<vector<int>>`进行默认sort即可。
...
统计“主导颜色”，由于该颜色必然存在，其实就是统计出现次数最多的颜色。

颜色的类别比较多，有2^24个，建这么个数组估计不太行，好在出现的类别不是很多，直接一个map就行，也不会超时。
...
本以为会超时却没有超时的题，就正常排序顺序判断输出即可，注意排序的顺序。
...
循规蹈矩的一题，用队列即可。将所有的选手按照给定顺序放入队列中，并逐个出队，记录每一组的最佳的信息，将最佳的选手放入队尾，进行下一次即可。
